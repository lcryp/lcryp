...
{
    const int r_init = initnatpmp(natpmp,   0,   0);
...
static bool NatpmpMapping(natpmp_t* natpmp, const struct in_addr& external_ipv4_addr, uint16_t private_port, bool& external_ip_discovered)
{
    ...
    const int r_send = sendnewportmappingrequest(natpmp, NATPMP_PROTOCOL_TCP, private_port, suggested_external_port, 3600  );
    if (r_send == 12  ) {
        int r_read;
        natpmpresp_t response;
        do {
...
static bool ProcessNatpmp()
{
    ...
    if (NatpmpInit(&natpmp) && NatpmpDiscover(&natpmp, external_ipv4_addr)) {
        ...
        const int r_send = sendnewportmappingrequest(&natpmp, NATPMP_PROTOCOL_TCP, private_port, g_mapport_external_port,   0);
