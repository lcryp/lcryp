...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-rpc", "Direct console commands.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-version", "Print version and exit", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", LCRYP_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: lcryp-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to lcrypd). This changes the RPC endpoint used, e.g. http://127.0.0.1:4994/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (gArgs.IsArgSet("-version")) {
            ...
            strUsage += "\n"
                "Usage:  lcryp-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     lcryp-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     lcryp-cli [options] help                List commands\n"
                "or:     lcryp-cli [options] help <command>      Get help for a command\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires lcrypd server to be running v22.0 and up");
        }
...
public:
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].getInt<int>() < 209900) {
            throw std::runtime_error("-netinfo requires lcrypd server to be running v0.21.0 and up");
        }
        ...
        for (const UniValue& peer : batch[ID_PEERINFO]["result"].getValues()) {
            ...
            ++m_counts.at(is_outbound).at(NETWORKS.size());
            ++m_counts.at(2).at(network_id);
            ++m_counts.at(2).at(NETWORKS.size());
        ...
        for (const UniValue& network : networkinfo["networks"].getValues()) {
            if (network["reachable"].get_bool()) {
                ...
                const int8_t network_id{NetworkStringToId(network_name)};
                if (network_id == UNKNOWN_NETWORK) continue;
                result += strprintf("%8s", network_name);
        ...
        for (size_t i = 0; i < rows.size(); ++i) {
            ...
            for (int8_t n : reachable_networks) {
                result += strprintf("%8i", m_counts.at(i).at(n));
            }
            result += strprintf("   %5i", m_counts.at(i).at(NETWORKS.size()));
            if (i == 1) {
    ...
    const std::string m_help_doc{
        ...
        "  version  Peer version and subversion concatenated, e.g. \"70016/Ryp:21.0.0/\"\n\n"
        ...
        "> lcryp-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> lcryp-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title lcryp-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "See this help\n"
        "> lcryp-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\nMake sure the lcrypd server is running and that you are connecting to the correct RPC port.", host, port, responseErrorMessage));
    } else if (response.status == HTTP_UNAUTHORIZED) {
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                "Could not locate RPC credentials. No authentication cookie could be found, and RPC password is not set.  See -rpcpassword and -stdinrpcpass.  Configuration file: (%s)",
                fs::PathToString(GetConfigFile(gArgs.GetPathArg("-conf", LCRYP_CONF_FILENAME)))));
        } else {
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to lcryp-cli command line.";
        }
    } else {
...
static void GetWalletBalances(UniValue& result)
{
    ...
    const UniValue listwallets = ConnectAndCallRPC(&rh, "listwallets",  {});
    ...
    for (const UniValue& wallet : wallets.getValues()) {
        ...
        const UniValue getbalances = ConnectAndCallRPC(&rh, "getbalances",  {}, wallet_name);
...
static UniValue GetNewAddress()
{
    ...
    DefaultRequestHandler rh;
    return ConnectAndCallRPC(&rh, "getnewaddress",  {}, wallet_name);
}
...
static int CommandLineRPC(int argc, char *argv[])
{
    ...
    try {
        ...
        std::string method;
        if (gArgs.GetBoolArg("-rpc", false)) {
            rh.reset(new DefaultRequestHandler());
            if (args.size() < 1) {
                throw std::runtime_error("Enter the command");
            }
            method = args[0];
            args.erase(args.begin());
        }
        else if (gArgs.IsArgSet( "-getinfo")) {
