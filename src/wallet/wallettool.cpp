...
static const std::shared_ptr<CWallet> MakeWallet(const std::string& name, const fs::path& path, const ArgsManager& args, DatabaseOptions options)
{
    ...
    std::shared_ptr<CWallet> wallet_instance{new CWallet(nullptr  , name, args, std::move(database)), WalletToolReleaseWallet};
...
bool ExecuteWalletToolFunc(const ArgsManager& args, const std::string& command)
{
    ...
    if (command == "create") {
        ...
        tfm::format(std::cout, "The dumpfile may contain private keys. To ensure the safety of your LcRyp, do not share the dumpfile.\n");
        return ret;
