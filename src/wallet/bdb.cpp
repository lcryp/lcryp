...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool lcryp-wallet to salvage or restoring a backup."), fs::quoted(fs::PathToString(file_path)));
            return false;
        }
    }
    return true;
    ...
    unsigned int nFlags = DB_THREAD | DB_CREATE;
    {
        ...
        if (m_db == nullptr) {
            ...
            ret = pdb_temp->open(nullptr,
                            fMockDb ? nullptr : strFile.c_str(),
                            fMockDb ? strFile.c_str() : "main",
                            DB_BTREE,
                            nFlags,
                            0);
            if (ret != 0) {
