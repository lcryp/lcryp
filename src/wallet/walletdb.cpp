...
DBErrors WalletBatch::LoadWallet(CWallet* pwallet)
{
    ...
    for (auto spk_man_pair : wss.m_active_external_spks) {
        pwallet->LoadActiveScriptPubKeyMan(spk_man_pair.second, spk_man_pair.first,  false);
    }
    for (auto spk_man_pair : wss.m_active_internal_spks) {
        pwallet->LoadActiveScriptPubKeyMan(spk_man_pair.second, spk_man_pair.first,  true);
    }
    ...
    try {
        ...
        while (true)
        {
            ...
            if (strType == DBKeys::TX) {
                ...
                vWtx.emplace_back( nullptr, TxStateInactive{});
                ssValue >> vWtx.back();
            }
        }
    } catch (...) {
