    ...
    bool fRescan = true;
    {
        ...
        CKeyID vchAddress = pubkey.GetID();
        {
            ...
            if (pubkey.IsCompressed()) {
                pwallet->ImportScripts({GetScriptForDestination(WitnessV0KeyHash(vchAddress))}, 0  );
            }
        }
    }
    if (fRescan) {
...
RPCHelpMan importaddress()
{
    return RPCHelpMan{"importaddress",
            ...
            "Note: This command is only compatible with legacy wallets. Use \"importdescriptors\" with \"addr(X)\" for descriptor wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The LcRyp address (or hex-encoded script)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
        ...
        fP2SH = request.params[3].get_bool();
    {
        ...
        if (IsValidDestination(dest)) {
            ...
            pwallet->MarkDirty();
            pwallet->ImportScriptPubKeys(strLabel, {GetScriptForDestination(dest)}, false  , true  , 1  );
        } else if (IsHex(request.params[0].get_str())) {
            std::vector<unsigned char> data(ParseHex(request.params[0].get_str()));
            CScript redeem_script(data.begin(), data.end());
            std::set<CScript> scripts = {redeem_script};
            pwallet->ImportScripts(scripts, 0  );
            if (fP2SH) {
                scripts.insert(GetScriptForDestination(ScriptHash(redeem_script)));
            }
            pwallet->ImportScriptPubKeys(strLabel, scripts, false  , true  , 1  );
        } else {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid LcRyp address or script");
        }
    }
    if (fRescan)
    {
        ...
        {
            LOCK(pwallet->cs_wallet);
            pwallet->ResubmitWalletTransactions( false,  true);
        }
    }
    return UniValue::VNULL;
},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Pubkey is not a valid public key");
    {
        ...
        pwallet->MarkDirty();
        pwallet->ImportScriptPubKeys(strLabel, script_pub_keys, true  , true  , 1  );
        pwallet->ImportPubKeys({pubKey.GetID()}, {{pubKey.GetID(), pubKey}} , {}  , false  , false  , 1  );
    }
    if (fRescan)
    {
        RescanWallet(*pwallet, reserver);
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fGood = true;
    {
        ...
        pwallet->chain().showProgress(strprintf("%s " + _("Importing…").translated, pwallet->GetDisplayName()), 0, false);
    ...
    pwallet->chain().showProgress("", 100, false);
    RescanWallet(*pwallet, reserver, nTimeBegin, false  );
    pwallet->MarkDirty();
    if (!fGood)
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The lcryp address for the private key"},
                },
                RPCResult{
...
static std::string RecurseImportData(const CScript& script, ImportData& import_data, const ScriptContext script_ctx)
{
    ...
    case TxoutType::WITNESS_V0_SCRIPTHASH: {
        ...
        if (script_ctx == ScriptContext::TOP) {
            import_data.import_scripts.emplace(script);
        }
        import_data.import_scripts.emplace(*subscript);
    ...
    if (have_solving_data) {
        ...
        if (error.empty()) {
            for (const auto& require_key : import_data.used_keys) {
                if (!require_key.second) continue;
...
static UniValue ProcessImportDescriptor(ImportData& import_data, std::map<CKeyID, CPubKey>& pubkey_map, std::map<CKeyID, CKey>& privkey_map, std::set<CScript>& script_pub_keys, bool& have_solving_data, const UniValue& data, std::vector<CKeyID>& ordered_pubkeys)
{
    ...
    std::string error;
    auto parsed_desc = Parse(descriptor, keys, error,   true);
    if (!parsed_desc) {
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "Response is an array with the same size as the input that has the execution result",
                    {
                        {RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::ARR, "warnings",  true, "",
                            {
                                {RPCResult::Type::STR, "", ""},
                            }},
                            {RPCResult::Type::OBJ, "error",  true, "",
                            {
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see lcrypd log for details) and could "
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& main_request) -> UniValue
{
    ...
    if (rescan) {
        int64_t scanned_time = pwallet->RescanFromTime(lowest_timestamp, reserver, true  );
        {
            LOCK(pwallet->cs_wallet);
            pwallet->ResubmitWalletTransactions( false,  true);
        }
        ...
        RPCResult{RPCResult::Type::OBJ, "", "", {
            ...
            {RPCResult::Type::ARR, "descriptors", "Array of descriptor objects (sorted by descriptor string representation)",
            {
                {RPCResult::Type::OBJ, "", "", {
                    ...
                    {RPCResult::Type::BOOL, "internal",  true, "True if this descriptor is used to generate change addresses. False if this descriptor is used to generate receiving addresses; defined only for active descriptors"},
