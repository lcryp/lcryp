...
static UniValue ListReceived(const CWallet& wallet, const UniValue& params, const bool by_label, const bool include_immature_coinbase) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (filtered_address) {
        const auto& entry = wallet.FindAddressBookEntry(*filtered_address,  false);
        if (entry) func(*filtered_address, entry->GetLabel(), entry->purpose,  false);
    } else {
    ...
    return RPCHelpMan{"listreceivedbyaddress",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        {RPCResult::Type::OBJ, "", "",
                        {
                            {RPCResult::Type::BOOL, "involvesWatchonly",  true, "Only returns true if imported addresses were involved in transaction"},
...
static const std::vector<RPCResult> TransactionDescriptionString()
{
    return{{RPCResult::Type::NUM, "confirmations", "The number of confirmations for the transaction. Negative confirmations means the\n"
           ...
           {RPCResult::Type::BOOL, "generated",  true, "Only present if the transaction's only input is a coinbase one."},
           {RPCResult::Type::BOOL, "trusted",  true, "Whether we consider the transaction to be trusted and safe to spend from.\n"
                "Only present when the transaction has 0 confirmations (or negative confirmations, if conflicted)."},
           {RPCResult::Type::STR_HEX, "blockhash",  true, "The block hash containing the transaction."},
           {RPCResult::Type::NUM, "blockheight",  true, "The block height containing the transaction."},
           {RPCResult::Type::NUM, "blockindex",  true, "The index of the transaction in the block that includes it."},
           ...
           {RPCResult::Type::STR_HEX, "replaced_by_txid",  true, "The txid if this tx was replaced."},
           {RPCResult::Type::STR_HEX, "replaces_txid",  true, "The txid if the tx replaces one."},
           {RPCResult::Type::STR, "comment",  true, ""},
           {RPCResult::Type::STR, "to",  true, "If a comment to is associated with the transaction."},
           {RPCResult::Type::NUM_TIME, "time", "The transaction time expressed in " + UNIX_EPOCH_TIME + "."},
           {RPCResult::Type::NUM_TIME, "timereceived", "The time received expressed in " + UNIX_EPOCH_TIME + "."},
           {RPCResult::Type::STR, "comment",  true, "If a comment is associated with the transaction, only present if not empty."},
           ...
           {RPCResult::Type::ARR, "parent_descs",  true, "Only if 'category' is 'received'. List of parent descriptors for the scriptPubKey of this coin.", {
    ...
    return RPCHelpMan{"listtransactions",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "",
                    {
                        ...
                        {
                            {RPCResult::Type::BOOL, "involvesWatchonly",  true, "Only returns true if imported addresses were involved in transaction."},
                            {RPCResult::Type::STR, "address", "The lcryp address of the transaction."},
                            ...
                            {RPCResult::Type::STR, "label",  true, "A comment for the address/transaction, if any"},
                            {RPCResult::Type::NUM, "vout", "the vout value"},
                            {RPCResult::Type::STR_AMOUNT, "fee",  true, "The amount of the fee in " + CURRENCY_UNIT + ". This is negative and only available for the\n"
                                 "'send' category of transactions."},
                        },
                        TransactionDescriptionString()),
                        {
                            {RPCResult::Type::BOOL, "abandoned",  true, "'true' if the transaction has been abandoned (inputs are respendable). Only available for the \n"
                                 "'send' category of transactions."},
                        })},
                    }
                },
    ...
    return RPCHelpMan{"listsinceblock",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::ARR, "removed",  true, "<structure is the same as \"transactions\" above, only present if include_removed=true>\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull() && !request.params[0].get_str().empty()) {
        ...
        height = int{};
        altheight = int{};
        if (!wallet.chain().findCommonAncestor(blockId, wallet.GetLastBlockHash(),   FoundBlock().height(*height),   FoundBlock().height(*altheight))) {
    ...
    for (const std::pair<const uint256, CWalletTx>& pairWtx : wallet.mapWallet) {
        ...
        if (depth == -1 || abs(wallet.GetTxDepthInMainChain(tx)) < depth) {
            ListTransactions(wallet, tx, 0, true, transactions, filter, nullptr  ,  include_change);
        }
    }
    ...
    while (include_removed && altheight && *altheight > *height) {
        ...
        for (const CTransactionRef& tx : block.vtx) {
            ...
            if (it != wallet.mapWallet.end()) {
                ListTransactions(wallet, it->second, -100000000, true, removed, filter, nullptr  ,  include_change);
            }
        }
                ...
                RPCResult{
                    ...
                    TransactionDescriptionString()),
                    {
                        {RPCResult::Type::ARR, "details", "",
                        {
                            ...
                            {
                                {RPCResult::Type::BOOL, "involvesWatchonly",  true, "Only returns true if imported addresses were involved in transaction."},
                                {RPCResult::Type::STR, "address",  true, "The lcryp address involved in the transaction."},
                                {RPCResult::Type::STR, "category", "The transaction category.\n"
                                    "\"send\"                  Transactions sent.\n"
                                    "\"receive\"               Non-coinbase transactions received.\n"
                                    "\"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
                        ...
                        {RPCResult::Type::OBJ, "decoded",  true, "The decoded transaction (only present when `verbose` is passed)",
                        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    CWallet::ScanResult result =
        pwallet->ScanForWalletTransactions(start_block, start_height, stop_height, reserver,  true,  false);
