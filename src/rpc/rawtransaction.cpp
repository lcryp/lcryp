...
static std::vector<RPCResult> DecodeTxDoc(const std::string& txid_field_doc)
{
    return {
        ...
        {RPCResult::Type::ARR, "vin", "",
        {
            {RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::STR_HEX, "coinbase",  true, "The coinbase value (only if coinbase transaction)"},
                {RPCResult::Type::STR_HEX, "txid",  true, "The transaction id (if not coinbase transaction)"},
                {RPCResult::Type::NUM, "vout",  true, "The output number (if not coinbase transaction)"},
                {RPCResult::Type::OBJ, "scriptSig",  true, "The script (if not coinbase transaction)",
                {
                    {RPCResult::Type::STR, "asm", "Disassembly of the signature script"},
                    {RPCResult::Type::STR_HEX, "hex", "The raw signature script bytes, hex-encoded"},
                }},
                {RPCResult::Type::ARR, "txinwitness",  true, "",
                {
                ...
                },
                {
                    RPCResult{"if verbose is not set or set to false",
                     ...
                     RPCResult{"if verbose is set to true",
                         ...
                         DecodeTxDoc( "The transaction id (same as provided)")),
                    },
                },
                RPCExamples{
...
static RPCHelpMan decoderawtransaction()
{
    return RPCHelpMan{"decoderawtransaction",
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    DecodeTxDoc( "The transaction id"),
                },
                RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (can_wrap) {
        ...
        if (can_wrap_P2WSH) {
            ...
            ScriptToUniv(segwitScr,  sr,  true,  true);
...
static RPCHelpMan decodepsbt()
{
    return RPCHelpMan{
        "decodepsbt",
        "Return a JSON object representing the serialized, base64-encoded partially signed LcRyp transaction.",
                {
...
static RPCHelpMan combinepsbt()
{
    return RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed LcRyp transactions into one transaction.\n"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    HidingSigningProvider public_provider(&provider,  true,  false);
    CCoinsView viewDummy;
    CCoinsViewCache view(&viewDummy);
    {
        NodeContext& node = EnsureAnyNodeContext(request.context);
    ...
    for (unsigned int i = 0; i < psbtx.tx->vin.size(); ++i) {
        ...
        SignPSBTInput(public_provider, psbtx, i, &txdata,  1);
    }
                    ...
                    {RPCResult::Type::ARR, "inputs",  true, "",
                    {
                        {RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::OBJ, "missing",  true, "Things that are missing that are required to complete this input",
                            {
                                ...
                                {RPCResult::Type::STR_HEX, "redeemscript",  true, "Hash160 of the redeemScript that is missing"},
                                {RPCResult::Type::STR_HEX, "witnessscript",  true, "SHA256 of the witnessScript that is missing"},
                            }},
