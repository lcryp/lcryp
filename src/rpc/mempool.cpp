...
static RPCHelpMan sendrawtransaction()
{
    return RPCHelpMan{"sendrawtransaction",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const TransactionError err = BroadcastTransaction(node, tx, err_string, max_raw_tx_fee,  true,  true);
    ...
    return RPCHelpMan{"testmempoolaccept",
        ...
        RPCResult{
                                      ...
                                      "Transactions that cannot be fully validated due to failures in other transactions will not contain an 'allowed' result.\n",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    ...
                    {RPCResult::Type::STR, "package-error",  true, "Package validation error, if any (only possible if rawtxs had more than 1 transaction)."},
                    {RPCResult::Type::BOOL, "allowed",  true, "Whether this tx would be accepted to the mempool and pass client-specified maxfeerate. "
                                                       "If not present, the tx was not fully validated due to a failure in another tx in the list."},
                    {RPCResult::Type::NUM, "vsize",  true, "Virtual transaction size as defined in BIP 141. This is different from actual serialized size for witness transactions as witness data is discounted (only present when 'allowed' is true)"},
                    {RPCResult::Type::OBJ, "fees",  true, "Transaction fees (only present if 'allowed' is true)",
                    {
                        {RPCResult::Type::STR_AMOUNT, "base", "transaction fee in " + CURRENCY_UNIT},
                    }},
                    {RPCResult::Type::STR, "reject-reason",  true, "Rejection string (only present when 'allowed' is false)"},
                }},
            }
        },
        RPCExamples{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const PackageMempoolAcceptResult package_result = [&] {
                LOCK(::cs_main);
                if (txns.size() > 1) return ProcessNewPackage(chainstate, mempool, txns,  true);
                return PackageMempoolAcceptResult(txns[0]->GetWitnessHash(),
                                                  chainman.ProcessTransaction(txns[0],  true));
            }();
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    ...
                    {RPCResult::Type::STR_HEX, "spendingtxid",  true, "the transaction id of the mempool transaction spending this output (omitted if unspent)"},
                }},
            }
        },
        RPCExamples{
...
static RPCHelpMan submitpackage()
{
    return RPCHelpMan{"submitpackage",
            ...
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::OBJ_DYN, "tx-results", "transaction results keyed by wtxid",
                {
                    {RPCResult::Type::OBJ, "wtxid", "transaction wtxid", {
                        {RPCResult::Type::STR_HEX, "txid", "The transaction hash in hex"},
                        {RPCResult::Type::STR_HEX, "other-wtxid",  true, "The wtxid of a different transaction with the same txid but different witness found in the mempool. This means the submitted transaction was ignored."},
                ...
                {RPCResult::Type::STR_AMOUNT, "package-feerate",  true, "package feerate used for feerate checks in " + CURRENCY_UNIT + " per KvB. Excludes transactions which were deduplicated or accepted individually."},
                {RPCResult::Type::ARR, "replaced-transactions",  true, "List of txids of replaced transactions",
                {
                    {RPCResult::Type::STR_HEX, "", "The transaction id"},
                }},
            },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const auto package_result = WITH_LOCK(::cs_main, return ProcessNewPackage(chainstate, mempool, txns,   false));
