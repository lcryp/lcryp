...
const char * const LCRYP_CONF_FILENAME = "lcryp.conf";
const char * const LCRYP_SETTINGS_FILENAME = "settings.json";
ArgsManager gArgs;
...
{
    fs::path settings = GetPathArg("-settings", LCRYP_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
bool ArgsManager::WriteSettingsFile(std::vector<std::string>* errors, bool backup) const
{
    ...
    if (!GetSettingsPath(&path,  false, backup) || !GetSettingsPath(&path_tmp,  true, backup)) {
...
{
    Assert((flags & ArgsManager::COMMAND) == 0);
...
{
#ifdef WIN32
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LcRyp";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    return pathRet / "Library/Application Support/LcRyp";
#else
    return pathRet / ".lcryp";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", LCRYP_CONF_FILENAME);
    ...
    if (stream.good()) {
        ...
        if (use_conf_file) {
            ...
            add_includes(chain_id,   chain_includes);
            add_includes({},   default_includes);
...
std::string ArgsManager::GetChainName() const
{
    auto get_net = [&](const std::string& arg) {
        LOCK(cs_args);
        util::SettingsValue value = util::GetSetting(m_settings,   "", SettingName(arg),
              false,
             false,
              true);
