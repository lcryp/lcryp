...
void PSBTOperationsDialog::openWithPSBT(PartiallySignedTransaction psbtx)
{
    ...
    if (m_wallet_model) {
        size_t n_could_sign;
        TransactionError err = m_wallet_model->wallet().fillPSBT(SIGHASH_ALL, false  , true  , &n_could_sign, m_transaction_data, complete);
...
void PSBTOperationsDialog::signTransaction()
{
    ...
    TransactionError err = m_wallet_model->wallet().fillPSBT(SIGHASH_ALL, true  , true  , &n_signed, m_transaction_data, complete);
    if (err != TransactionError::OK) {
...
void PSBTOperationsDialog::saveTransaction() {
    ...
    for (const CTxOut& out : m_transaction_data.tx->vout) {
        ...
        QString amount = LcRypUnits::format(m_client_model->getOptionsModel()->getDisplayUnit(), out.nValue);
...
std::string PSBTOperationsDialog::renderTransaction(const PartiallySignedTransaction &psbtx)
{
    ...
    for (const CTxOut& out : psbtx.tx->vout) {
        ...
        tx_description.append(tr(" * Sends %1 to %2")
            .arg(LcRypUnits::formatWithUnit(LcRypUnit::LCR, out.nValue))
    ...
    if (!*analysis.fee) {
        ...
        tx_description.append(LcRypUnits::formatWithUnit(LcRypUnit::LCR, *analysis.fee));
        tx_description.append("<hr />");
        QStringList alternativeUnits;
        for (const LcRypUnits::Unit u : LcRypUnits::availableUnits())
        {
        ...
        tx_description.append(QString("<b>%1</b>: <b>%2</b>").arg(tr("Total Amount"))
            .arg(LcRypUnits::formatHtmlWithUnit(m_client_model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
size_t PSBTOperationsDialog::couldSignInputs(const PartiallySignedTransaction &psbtx) {
    ...
    size_t n_signed;
    bool complete;
    TransactionError err = m_wallet_model->wallet().fillPSBT(SIGHASH_ALL, false  , false  , &n_signed, m_transaction_data, complete);
    if (err != TransactionError::OK) {
        return 0;
    }
    return n_signed;
    ...
    switch (analysis.next) {
        ...
        case PSBTRole::SIGNER: {
            ...
            if (!m_wallet_model) {
                ...
                need_sig_text += " " + tr("(But this wallet does not have the right keys.)");
                level = StatusLevel::WARN;
            }
