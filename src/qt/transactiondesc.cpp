...
QString TransactionDesc::toHTML(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, LcRypUnit unit)
{
    int numBlocks;
    ...
    else
    {
        if (nNet > 0)
        {
            ...
            if (IsValidDestination(address)) {
                std::string name;
                isminetype ismine;
                if (wallet.getAddress(address, &name, &ismine,   nullptr))
                {
    ...
    if (wtx.value_map.count("to") && !wtx.value_map["to"].empty())
    {
        ...
        if (wallet.getAddress(
                dest, &name,   nullptr,   nullptr) && !name.empty())
    ...
    if (wtx.is_coinbase && nCredit == 0)
    {
        ...
        if (status.is_in_main_chain)
            strHTML += LcRypUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", status.blocks_to_maturity) + ")";
        else
    ...
    {
        strHTML += "<b>" + tr("Credit") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                if (!wtx.value_map.count("to") || wtx.value_map["to"].empty())
                {
                    ...
                    if (ExtractDestination(txout.scriptPubKey, address))
                    {
                        ...
                        std::string name;
                        if (wallet.getAddress(
                                address, &name,   nullptr,   nullptr) && !name.empty())
                            strHTML += GUIUtil::HtmlEscape(name) + " ";
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            if (fAllToMe)
            {
                CAmount nChange = wtx.change;
                CAmount nValue = nCredit - nChange;
                strHTML += "<b>" + tr("Total debit") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }
            CAmount nTxFee = nDebit - wtx.tx->GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            ...
            for (const CTxIn& txin : wtx.tx->vin) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Debit") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, -wallet.getDebit(txin, ISMINE_ALL)) + "<br>";
                }
            }
            mine = wtx.txout_is_mine.begin();
            for (const CTxOut& txout : wtx.tx->vout) {
                if (*(mine++)) {
                    strHTML += "<b>" + tr("Credit") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, wallet.getCredit(txout, ISMINE_ALL)) + "<br>";
                }
            }
        }
    }
    strHTML += "<b>" + tr("Net amount") + ":</b> " + LcRypUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
            ...
            {
                {
                    ...
                    {
                        std::string name;
                        if (wallet.getAddress(address, &name,   nullptr,   nullptr) && !name.empty())
                            strHTML += GUIUtil::HtmlEscape(name) + " ";
                        strHTML += QString::fromStdString(EncodeDestination(address));
                    }
                    strHTML = strHTML + " " + tr("Amount") + "=" + LcRypUnits::formatHtmlWithUnit(unit, vout.nValue);
