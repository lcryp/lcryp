...
uint16_t GetListenPort()
{
    for (const std::string& bind_arg : gArgs.GetArgs("-bind")) {
        ...
        if (Lookup(bind_arg, bind_addr, dummy_port,  false)) {
...
std::vector<CAddress> CConnman::GetAddresses(CNode& requestor, size_t max_addresses, size_t max_pct)
{
    ...
    if (cache_entry.m_cache_entry_expiration < current_time) {
        cache_entry.m_addrs_response_cache = GetAddresses(max_addresses, max_pct,  std::nullopt);
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    if (gArgs.GetBoolArg("-capturemessages", false)) {
        CaptureMessage(pnode->addr, msg.m_type, msg.data,  false);
    }
