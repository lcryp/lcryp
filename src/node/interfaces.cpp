...
public:
    ...
    UniValue miningGUI(bool fGenerate, int nGenProcLimit, const std::string s_wallet) override
    {
        try {
            if (nGenProcLimit <= 0)
            {
            ...
            UniValue _params(UniValue::VARR);
            if (fGenerate)
                _params.push_back("1");
            else
                _params.push_back("0");
            _params.push_back(s_wallet);
            _params.push_back(std::to_string(nGenProcLimit));
            JSONRPCRequest req;
            req.context = m_context;
            req.params = _params;
            req.strMethod = "setmining";
            req.URI = "";
    ...
    bool baseInitialize() override
    {
        ...
        if (!AppInitParameterInteraction(gArgs,  false)) return false;
    ...
    {
        gArgs.WriteSettingsFile( nullptr,  true);
        gArgs.LockSettings([&](util::Settings& settings) {
    ...
    {
        return BroadcastTransaction(*m_context, std::move(tx), err_string, max_tx_fee,   true,   false);
    }
...
public:
    ...
    {
        const TransactionError err = BroadcastTransaction(m_node, tx, err_string, max_tx_fee, relay,  false);
