#!/usr/bin/env python3
# Copyright (c) 2016-2021 The LcRyp Core developers
...
def parse_config_into_lcr_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"LcRyp Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/lcryp_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/lcryp_config.h'), "w", encoding="utf8") as lcr_config:
        lcr_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='LcRyp-core msbuild configuration initialiser.')
    ...
    parse_config_into_lcr_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/lcryp_config.h'), os.path.join(SOURCE_DIR, 'config/lcryp-config.h'))
